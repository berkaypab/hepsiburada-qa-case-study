...
class CustomReporter implements Reporter {
    ...
    onTestBegin(test: TestCase) {
        ...
        console.log(`${COLORS.blue}${ICONS.play}  STARTED:${COLORS.reset} [${COLORS.magenta}${project}${COLORS.reset}] > ${test.title}`);
    }
    ...
    onTestEnd(test: TestCase, result: TestResult) {
        ...
        if (result.status === "passed") {
            this.stats.passed++;
            console.log(`${COLORS.green}${ICONS.check}  PASSED:${COLORS.reset} [${project}] > ${test.title} (${COLORS.yellow}${duration}${COLORS.reset})\n`);
        } else if (result.status === "skipped") {
            this.stats.skipped++;
            console.log(`${COLORS.yellow}${ICONS.skip}  SKIPPED:${COLORS.reset} [${project}] > ${test.title}\n`);
        } else if (result.status === "failed" || result.status === "timedOut") {
            this.stats.failed++;
            console.log(`${COLORS.red}${ICONS.cross}  FAILED:${COLORS.reset} [${project}] > ${test.title} (${COLORS.yellow}${duration}${COLORS.reset})`);
    ...
    onEnd(result: FullResult) {
        ...
        console.log(`  Tests:    ${COLORS.green}${this.stats.passed} Passed${COLORS.reset} | ${COLORS.red}${this.stats.failed} Failed${COLORS.reset} | ${COLORS.yellow}${this.stats.skipped} Skipped${COLORS.reset} | ${this.stats.total} Total`);
        console.log(`  Duration: ${COLORS.yellow}${totalDuration}s${COLORS.reset} | Success Rate: ${this.getRateColor(successRate)}${successRate}%${COLORS.reset}`);
        console.log(`${COLORS.bright}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${COLORS.reset}\n`);
    ...
    private getStatusColor(status: string): string {
        switch (status) {
            ...
            case "failed":
            case "timedOut": return COLORS.red;
